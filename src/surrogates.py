...
def weight_preserved(G: MLGraph, weight: str, n_swaps: int=5, 
                     preserve_layer: bool=False) -> MLGraph:
    """Generate a weighted surrogate multilayer graph from a given weighted 
    multilayer graph by swapping its edges. The method selects an edge (a, b,
    w_ab) and two nodes c and d; where w_ab is the edge weight and a, b, c, and
    d are all distinct. If (c, d, w_cd) is not an edge, it removes (a, b, w_ab)
    and add the edge (c, d, w_ab) to the graph. If (c, d, w_cd) is an edge, it
    swaps edge weights of two edges. Check [1] for more details. If preserve
    layer is True, it is ensured that layers of a and c and layers of b and d
    are the same.

    Parameters
    ----------
    G : MLGraph
        The multilayer graph from which the null multilayer graph will be
        generated.
    weight: str
        The edge attribute to use as edge weight.
    n_swaps : int, optional
        Number of times to swap each edge on average, by default 5
    ...
    for _ in range(n_swaps):
        ...
        while attempt <= max_attempt:
            ...
            # select a pair of nodes different than (a, b) to consider for swap
            # with (a, b)
            if preserve_layer:
